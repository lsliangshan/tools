...
var JSONFormat = window.JSONFormat = (function(){
  ...
  var hide = window.hide = function (obj) {
    ...
    if (data_type === 'array') {
      obj.parentNode.innerHTML = '<i  style="cursor:pointer; margin-left: 5px; margin-right: 5px;" class="fa fa-plus-square-o" onclick="show(this)"></i>Array[<span class="json_number">' + data_size + '</span>]';
    }else{
      obj.parentNode.innerHTML = '<i  style="cursor:pointer; margin-left: 5px; margin-right: 5px;" class="fa fa-plus-square-o" onclick="show(this)"></i>Object{...}';
    }

  }

  var hideAll = window.hideAll = function () {
    var allHideIcon = document.querySelectorAll('i.fa-minus-square-o')
    for (var i = 1; i < allHideIcon.length; i++) {
      hide(allHideIcon[i])
    }
  }

  var show = window.show = function (obj){
  ...
  var showAll = window.showAll = function () {
    var allShowIcon = document.querySelectorAll('i.fa-plus-square-o')
    for (var i = 0; i < allShowIcon.length; i++) {
      show(allShowIcon[i])
    }
  }
  ...
  function _format_array(object, indent_count){
    ...
    return '<span data-type="array" data-size="' + tmp_array.length + '"><i  style="cursor:pointer; margin-left: 5px; margin-right: 5px;" class="fa fa-minus-square-o" onclick="hide(this)"></i>[<br/>'
      + tmp_array.join(',<br/>')
  ...
  function _format_object(object, indent_count){
    ...
    return '<span  data-type="object"><i style="cursor:pointer; margin-left: 5px; margin-right: 5px;" class="fa fa-minus-square-o" onclick="hide(this)"></i>{<br/>'
      + tmp_array.join(',<br/>')
      + '<br/>' + indent_tab(indent_count - 1) + '}</span>';
  }
